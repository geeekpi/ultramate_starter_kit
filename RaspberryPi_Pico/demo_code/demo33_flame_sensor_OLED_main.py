...
flame_sensor = ADC(Pin(26))  # define flame sensor via AO pin


while True:
    oled.text("Flame sensor", 0, 0)
    oled.show()
    oled.text("V: {}".format(flame_sensor.read_u16()), 20, 30)
    oled.show()
    sleep(0.1)
    oled.fill(0)
    oled.show()
    if flame_sensor.read_u16() <= 30000:
        oled.text("Fire alarm!!", 0, 0)
        oled.blit(fb, 20, 30)
        oled.show()
        sleep(1)
        oled.fill(0)
